...
export default function HeritageRoadmap({ activeJourneyStep, setActiveJourneyStep }: HeritageRoadmapProps) {
  ...
  return (
    <section 
      id="heritage-timeline-voyage" 
      className="py-24 bg-stone-900 text-white relative overflow-hidden text-left"
    >
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-heritage-sand/5 via-transparent to-transparent pointer-events-none" />
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Header Block */}
        <div className="mb-16">
          <FadeIn direction="up">
            <Badge variant="sand" size="sm" className="mb-2 bg-heritage-sand/10 border-heritage-sand/30 text-heritage-sand">
        ...
        {/* Desktop/Tablet Horizontal Milestone Selector */}
        <div className="hidden md:flex items-center justify-between border-b border-stone-800 pb-8 mb-12 overflow-x-auto gap-4 scrollbar-none">
          {journeySteps.map((step, idx) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center gap-3 focus:outline-none transition-all group shrink-0 py-2 cursor-pointer outline-none ${
                  isActive ? 'scale-105' : 'hover:scale-102'
                }`}
                id={`journey-step-btn-${step.stepNumber}`}
                aria-label={`Select Milestone ${step.stepNumber}: ${step.title}`}
                aria-current={isActive ? 'true' : 'false'}
              >
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all border ${
                  isActive 
                    ? 'bg-heritage-sand border-heritage-sand text-stone-950 shadow-[0_0_20px_rgba(221,161,94,0.3)]' 
                    : isCompleted
                      ? 'bg-stone-800 border-heritage-sand/40 text-heritage-sand'
                      : 'bg-stone-950 border-stone-800 text-stone-500 group-hover:border-stone-700'
                } focus-visible:ring-2 focus-visible:ring-heritage-sand`}>
                  {isCompleted ? <Check className="w-5 h-5" /> : <Icon className="w-5 h-5" />}
                </div>
                <div className="text-center">
                  <span className={`block font-mono text-[9px] font-black uppercase tracking-wider ${isActive ? 'text-heritage-sand' : 'text-stone-500'}`}>
                    Milestone 0{step.stepNumber}
                  </span>
                  <span className={`block text-xs font-bold mt-0.5 truncate max-w-[120px] ${isActive ? 'text-white font-black' : 'text-stone-400 group-hover:text-stone-300'}`}>
                    {step.title}
                  </span>
                </div>
              </button>
        ...
        {/* Mobile Horizontal Milestone Selector */}
          ...
          {journeySteps.map((step, idx) => {
            ...
            return (
              <button
                key={step.stepNumber}
                onClick={() => setActiveJourneyStep(idx)}
                className={`px-4 py-2.5 rounded-xl border font-bold shrink-0 text-xs transition-colors cursor-pointer outline-none ${
                  isActive 
                    ? 'bg-heritage-sand border-heritage-sand text-stone-950' 
        ...
        <FadeIn direction="up">
          <div className="bg-stone-950 border border-stone-800 rounded-[32px] p-8 md:p-12 min-h-[380px] flex flex-col md:flex-row gap-12 items-center relative overflow-hidden shadow-2xl">
            <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-bl from-heritage-sand/[0.02] to-transparent pointer-events-none" />
            
            {/* Left stage details (Text Content) */}
            <div className="md:w-1/2 space-y-6">
              <div className="inline-flex items-center gap-2 bg-heritage-sand/10 border border-heritage-sand/20 px-3 py-1.5 rounded-lg text-heritage-sand font-mono text-[10px] font-bold uppercase tracking-wider">
              ...
              <div className="flex items-center gap-2 text-stone-400 font-mono text-xs font-bold uppercase tracking-wider">
                <span className="w-1.5 h-1.5 bg-heritage-sand rounded-full" />
            ...
            <div className="md:w-1/2 w-full h-[250px] md:h-[400px] rounded-2xl overflow-hidden relative border border-stone-800 shadow-2xl group">
              ...
              <div className="absolute bottom-6 left-6 right-6">
                <span className="text-[10px] text-heritage-sand font-mono uppercase tracking-wider font-bold">Visual Heritage Landmark</span>
